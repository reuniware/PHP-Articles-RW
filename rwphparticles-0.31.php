...
if (isset($_POST['password']) && isset($_POST['confirmpassword']) ) {
	...
	if (trim($password) == '' || trim($confirmpassword) == ''){
		if (DEBUG) echo 'Password of Confirm password is empty.<br/>';
		exit;
	}
	
	if ($password != $confirmpassword){
		if (DEBUG) echo 'Cannot create admin account because password do not match.<br/>';
		exit;
	}
	if ($password == $confirmpassword){
		$hashed = md5($password);
...
if (isset($_POST['login']) && isset($_POST['password'])){
	...
	if (DEBUG) echo 'login and password received in POST : ' . $login . " ; " . $passwordhash . '<br/>';
	...
	if ($r->num_rows > 0){
		if (DEBUG) echo 'Login attempt OK. Creating session.<br/>';
		$_SESSION['passwordhash'] = $passwordhash;
		$_SESSION['login'] = trim(strtolower($login));
	} else {
		if (DEBUG) echo 'Login attempt NOT OK.<br/>';
	}
	$db->close();
}
...
if ( isset($_SESSION['passwordhash']) && isset($_SESSION['login']) ){
	...
	$r = mysqli_query($db, "select * from " . MAIN_TABLE_NAME . "_" . "user where username='" . $login . "' and password='" . $passwordhash . "'");
	if ($r->num_rows > 0){
		if (DEBUG) echo 'Session existing and credentials OK.<br/>';
		$allowed = true;
	} else {
		if (DEBUG) echo 'Session existing and credentials NOT OK.<br/>';
		if (DEBUG) echo 'Credentials received : login = ' . $login . ' and passwordhash = ' . $passwordhash . "<br/>";
		$allowed = false;
	}
...
if ($allowed == true){
	if (isset($_POST['adminfunction'])){
		...
		if (DEBUG) echo '003:allowed is true ; adminfunction<br/>';
		if (DEBUG) echo 'adminfunction = ' . $adminfunction .'<br/>';
...
if (isset($_GET['disconnect'])){
	if (trim(strtolower($_GET['disconnect'])) == "true"){
		...
		if (DEBUG) echo 'Ctredentials in session have been reset.<br/>';
		$allowed = false;
		if (DEBUG) echo 'allowed is becoming FALSE.<br/>';
	}
}
...
if ( isset($_POST['titre']) && isset($_POST['contenu']) ){
	...
	if (DEBUG) echo 'titre et contenu reçus en POST.<br/>';
    if (DEBUG) echo "titre = " . $titre . " et contenu = " . $contenu . "<br/>";
...
if ($allowed == true){
	...
	echo '<table>';
	echo ' <tr><td>Titre de l\'article: </td><td><input type="text" id="titre" name="titre" value="" size=64><br/></td></tr>';
	echo ' <tr><td>Contenu de l\'article: </td><td><input type="text" id="contenu" name="contenu" value="" size=64></td></tr>';
	...
	echo '<br/>';
	echo ' <tr>&nbsp;<td></td><td><input id="btnsubmitarticle" type="submit" value="Submit"></td></tr>';
	echo '</table>';
	echo '</form></center>'; 
	echo '<br/><br/>';
...
if ($allowed == false){
	...
	echo "<form action='" . $_POST['current_file_name'] . "' method='post'>";
	echo '<table>';
	echo ' <tr><td>Login: </td><td><input type="text" id="login" name="login" value="" size=16><br/></td></tr>';
	echo ' <tr><td>Password: </td><td><input type="text" id="password" name="password" value="" size=16></td></tr>';
	echo ' <tr><td>&nbsp;</td><td><input type="submit" value="Connect"></td></tr>';
	echo '</table>';
	echo '</form></center>'; 
	echo '<br/><br/>';
